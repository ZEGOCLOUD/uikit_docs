...
- (void)viewDidLoad {
    ...
    // Create Voice Call invitation Button
    ZegoUIKitUser *callTargetUser = [[ZegoUIKitUser alloc] initWithUserID:@"TargetUserID" userName:@"TargetUserName"];
    ...
    [sendVoiceCallButton.inviteeList addObject:callTargetUser];
    sendVoiceCallButton.resourceID = @"zegouikit_call";
    sendVoiceCallButton.frame = CGRectMake(100, 100, 150, 40);
    // Add the button to your view
    [self.view addSubview:sendVoiceCallButton];
}
```
...
- (void)viewDidLoad {
    ...
    // Create video call invitation button.
    self.sendVideoCallButton = [[ZegoSendCallInvitationButton alloc] initWithInvitationType:ZegoInvitationTypeVideoCall];
    self.sendVideoCallButton.frame = CGRectMake(100, 100, 100, 30);
    [self.sendVideoCallButton setText:@"Video"];
    self.sendVideoCallButton.resourceID = @"xxx";
...
<Accordion title="Custom Call Button" defaultOpen="false">
