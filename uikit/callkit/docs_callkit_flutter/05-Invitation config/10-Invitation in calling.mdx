...
- ZegoCallInvitationConfig
  ...
  - bool `endCallWhenInitiatorLeave`:
    ...
    >  whether the entire call should end when the initiator leaves the call (it means that other participants also leave together).
    >
    >  Default value is `false`.
    >
    >  If it is set to `false`, the call can continue even after the initiator leaves.
