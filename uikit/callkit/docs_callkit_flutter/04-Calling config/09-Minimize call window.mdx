...
we will enable PIP by default, which means **enableWhenBackground** default value is true. If you don't want to enable it, you can modify it to false through **ZegoCallPIPConfig**. 

In addition, if you want to control it through buttons, you can add **ZegoCallMenuBarButtonName.pipButton** in **topMenuBar.buttons**.
...
<CodeGroup>
```dart title="With call invitation" {12}
ZegoUIKitPrebuiltCallInvitationService().init(
  ...
  requireConfig: (ZegoCallInvitationData data) {
    var config = (data.invitees.length > 1)
