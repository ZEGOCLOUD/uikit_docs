...
| config    | ZegoUIKitPrebuiltCallInvitationConfig | Yes      | 这可以用来设置呼叫邀请相关的配置。|                                                                                 |
...
| resourceID  | String | No       | `resourceID` 可以指定离线呼叫邀请的铃声，必须设置为与[ZEGOCLOUD管理控制台](https://console.zegocloud.com)中的推送资源ID相同的值。这只在notifyWhenAppRunningInBackgroundOrQuit为true时生效 |
