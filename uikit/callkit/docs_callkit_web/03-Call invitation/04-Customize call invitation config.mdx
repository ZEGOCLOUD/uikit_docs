...
By default, a call continues after the initiator leaves. To configure the call to automatically end when the initiator exits, set the `CallInvitationConfig.endCallWhenInitiatorLeave` property to `true`.

<Note title="Note">
To use this feature, please upgrade Call Kit to v2.9.0 or a newer version.
</Note>
