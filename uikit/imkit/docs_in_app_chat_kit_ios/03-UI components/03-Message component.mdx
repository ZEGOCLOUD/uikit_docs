...
To customize the button on the bottom toolbar, you can use it`smallButtons` `expandButtons` Configuration:
...
// More expanded button configurations, to take effect smallButtons must contain .expand
config.bottomConfig.expandButtons.append(.voiceCall)  
```
