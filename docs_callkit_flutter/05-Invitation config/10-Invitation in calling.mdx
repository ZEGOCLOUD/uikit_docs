...
If you use this feature(when **canInvitingInCalling** is true.), the invitation feature will no longer be compatible with versions before v4.12.0, that is mean, invitations will not be received between each other.

so when you release the App, please use Force Upgrade to prompt your App clients to upgrade to the latest version you released, otherwise there will be a problem of not receiving calls.
...
- ZegoCallInvitationConfig
 ...
 -  bool `canInvitingInCalling`:
    ...
    >  Please note that if allowed, it will be incompatible with versions before v5.0, which means mutual invitations cannot be made.
