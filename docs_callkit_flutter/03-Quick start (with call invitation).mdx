  ...
  <Tab title="Android">
    ...
    <Accordion title="3. Set the Kotlin & Gradle versions" defaultOpen="false">
      <Steps>
        ...
        <Step title="Make the `gradle` version >= 7.0: In the `your_project/android/gradle/wrapper/gradle-wrapper.properties` file, modify the `distributionUrl` to [`https://services.gradle.org/distributions/gradle-7.4-all.zip`.](https://services.gradle.org/distributions/gradle-7.4-all.zip.)">
          ```xml
...
<UIKIT_ANDROID_PREVENT_CODE_OBFUSCATION />
    ...
    <Accordion title="8. Guide your users to set app permissions (Android only)" defaultOpen="false">
