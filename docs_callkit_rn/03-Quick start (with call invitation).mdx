...
For Callkit's offline push, system notification permission is required. Starting from Android 13, you need to actively apply for notification permission. However, if the React Native version is lower than 0.71.0, applying for permission will result in a `permission is null` error. 
Here are two solutions:

#### Upgrade React Native Version
Upgrade React Native version to 0.71.0 and above, [reference link](https://react-native-community.github.io/upgrade-helper)

#### Manual Editing of React Native Source Code
...
2. Open `PermissionAndroid.js` file

3. Add the following code to `Permissions = Object.freeze({})`

```js
POST_NOTIFICATIONS: 'android.permission.POST_NOTIFICATIONS',
```

4. In class `PermissionsAndroid` add the following code.

```js
POST_NOTIFICATIONS: string,
```

5. Now go to build.gradle in android folder and change compileSDK and targetSDK version to 33.
