...
1. Call the [ZIMCombineMessage](@-ZIMCombineMessage) operation to construct a combined message. The following table lists the parameters.
    ...
    | ---- | ----| -------| -----|
    | type| number| Yes| It must be set to **100**, which indicates a combined message. |
    | title| string| Yes| The title of the combined message, which can contain up to 20 bytes in length. If you need a longer length, contact ZEGOCLOUD technical support.|
    | summary| string| Yes| The summary of the combined message, which can contain up to 100 bytes in length. If you need a longer length, contact ZEGOCLOUD technical support.|
