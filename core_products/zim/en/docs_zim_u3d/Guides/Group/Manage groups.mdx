...
After Client A logs in, he can call the [CreateGroup](@CreateGroup) method to create a new group, and he can be told whether the group is created successfully through the callback [ZIMGroupCreatedCallback](@ZIMGroupCreatedCallback). For related error codes, see [Error codes](!Error_Codes).
...
- You will automatically become the group owner after you create a group successfully. To transfer the ownership, refer to chapter **Transfer the group ownership** of the [Manage group members](!Group_Members).
- After creating a group, for the group owner to send messages to a group, call the [SendMessage](@SendMessage) method. For details, see [Send & Receive messages](!sendMessages#5).
</Warning>
...
- After joining a group, for the group members to send messages to a group, call the [SendMessage](@SendMessage) method. For details, see [Send & Receive messages](!sendMessages#5).
</Warning>
