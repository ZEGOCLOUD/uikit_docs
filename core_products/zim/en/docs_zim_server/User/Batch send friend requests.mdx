...
<td>Code</td>
<td>Number</td>
<td>The return code.<Note title="Note">When you send friend requests to multiple users at a time, take note of the following items:<ul><li>If one or more requests are successfully sent, `0` is returned. For more information, see `ErrorList`.</li><li>If all friend requests fail to be sent, a corresponding code is returned. For more information, see <a href="!Server_APIs/Return_Codes" target="_blank">Return codes</a>.</li></ul></Note></td>
</tr>
<tr data-row-level="3">
<td>Message</td>
<td>String</td>
