...
And client A can be notified whether the message is delivered successfully through the callback [ZIMMessageSentCallback](https://www.zegocloud.com/article/apidoc=zim_API~cpp_windows~interface~ZIMDefines#zim-message-sent-callback) (correspond to the `send_callback` shows below).
