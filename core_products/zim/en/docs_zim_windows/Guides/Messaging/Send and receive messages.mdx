...
The interface used to send custom messages is [sendMessage](@sendMessage), which is the same as the interface used to send normal messages. Developers can refer to [Sending Messages in Sending and Receiving Normal Messages](!IM-sendMessages#4_1) for details of this interface parameters.
...
The callback interface for receiving custom messages is the same as the callback interface for receiving normal messages. Please refer to [Receiving Messages in Sending and Receiving Normal Messages](!IM-sendMessages#4_2) for the specific interface.
...
The callback interface for receiving @ messages is the same as the callback interface for receiving regular messages. Please refer to [Send & Receive messages - Receive messages](!zim-sendMessages#4_2) for details on the specific interface.
...
When a user is mentioned, the [conversationChanged](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-conversation-changed) will be received, allowing you to retrieve the latest [mentionedInfoList](@mentionedInfoList-ZIMConversation) for the current [ZIMConversation](@-ZIMConversation).
...
- [clearConversationUnreadMessageCount](@clearConversationUnreadMessageCount): Clears the unread message count of a single conversation. Refer to the example code in [Manage sessions - Clear single session message unread](!zim-Conversation_Management#clearConversationUnreadMessageCount).
- [clearConversationTotalUnreadMessageCount](@clearConversationTotalUnreadMessageCount): Clears the total unread message count of all conversations. Refer to the example code in [Manage sessions - Clear all unread conversation messages](!zim-Conversation_Management#clearConversationTotalUnreadMessageCount).
...
Please refer to the server-side API documentation [Push message to all user](!zim-PushMessageToAllUsers) to learn how to send messages to all users from the server side.
...
ZIM SDK supports converting user operations within a session into Tips messages. When a related operation occurs, ZIM SDK will send a Tips message to the session to notify. For details, please refer to [Receive tip messages](!zim-receive_tips).
