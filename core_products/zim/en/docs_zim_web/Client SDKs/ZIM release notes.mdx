...
| New parameters for server-side create group interface | Added `CreateGroupTime` parameter to support setting events for group creation. | [Create Group](@IM-CreateGroup) |
| Added new parameters to the server-side interface for adding group members | Added the `GroupMemberInfos` parameter to support defining the group joining time and group joining mode of users who join the group. | [Create Group](@IM-CreateGroup) |
