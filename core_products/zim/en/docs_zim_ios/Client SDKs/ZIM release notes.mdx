...
| Feature  | Description | Method |
|-------|-------|-------|
| Multi-device login | The "multi-device login" strategy supports configuring the logic for kicking each other out. Currently, it only supports kicking out between Android and iOS devices, as well as between Windows and Mac devices. | [loginWithUserID](https://www.zegocloud.com/article/apidoc=zim_API~objective-c_ios~protocol~ZIM#login-with-user-id-config-callback-2) |
| Offline login | Supports users to login to the IM service and access local SDK data while being offline. | [loginWithUserID](https://www.zegocloud.com/article/apidoc=zim_API~objective-c_ios~protocol~ZIM#login-with-user-id-config-callback-2) |
...
| Call Invitation | Add callback [callInvitationCreated](https://www.zegocloud.com/article/apidoc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-invitation-created-call-id). The initiator of the call invitation can know that the call invitation has been created by listening to this callback. | [callInvitationCreated](https://www.zegocloud.com/article/apidoc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-invitation-created-call-id) |
...
| Item  | Description | Method |
| -----  | ---- | ----- |
| Call invitation reception callback |  Optimize the regular mode of call invitation, support offline users to receive call invitations and be immediately notified upon going online within the timeout period of the call invitation.| [callUserStateChanged](https://www.zegocloud.com/article/apidoc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-user-state-changed-call-id) |

<h4>Deprecation</h4>

| Method | Description | Change version |
| -----  | ---- | ----- |
| <ul><li>[callInvitationAccepted](https://www.zegocloud.com/article/apidoc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-invitation-accepted-info-call-id)</li><li>[callInvitationRejected](https://www.zegocloud.com/article/apidoc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-invitation-rejected-call-id)</li><li>[callInviteesAnsweredTimeout](https://www.zegocloud.com/article/apidoc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-invitees-answered-timeout-call-id)</li></ul> | To facilitate developers to monitor the call status changes of users in call invitations, [callUserStateChanged](https://www.zegocloud.com/article/apidoc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-call-user-state-changed-call-id) is added to replace the original call invitation callbacks `callInvitationAccepted`、`callInvitationRejected` and `callInviteesAnsweredTimeout`. | 2.9.0 |
