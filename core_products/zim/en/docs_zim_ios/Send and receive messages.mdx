...
After client B logs in, he will receive client A's message through the callback [receivePeerMessage](https://www.zegocloud.com/article/apidoc=zim_API~objectivec_ios~protocol~ZIMEventHandler#zim-receive-peer-message-from) which is already set in the [setEventHandler](@setEventHandler) method.
