...
- 30 seconds before a Token expires, the SDK sends out a notification through the {getPlatformData(props,onTokenWillExpireMap)} callback.
...
| Method | Description |
|-------|--------|
| [login](@login) | Log in to ZIM service. |
| [renewToken](@renewToken) | Renew authentication token. |
| [onTokenWillExpire](@onTokenWillExpire) | Token expiration callback. |
</div>
