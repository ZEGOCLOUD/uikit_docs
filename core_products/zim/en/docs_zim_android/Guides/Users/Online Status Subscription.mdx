...
Subsequently, the online status changes of the target users within the specified `subscriptionDuration` will be returned through the {getPlatformData(props,onUserStatusUpdatedMap)} event callback in {getPlatformData(props,ZIMEventHandlerMap)}.
