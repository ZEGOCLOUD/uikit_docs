...
export const unsubscribeUsersStatusMap = {
  ...
  'Flutter': <a href='https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIM/unsubscribeUsersStatus.html' target='_blank'>subscribeUsersStatus</a>,
}
...
export const querySubscribedUserStatusListMap = {
  ...
  'Flutter': <a href='https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIM/querySubscribedUserStatusList.html' target='_blank'>queryUsersStatus</a>,
}
...
export const ZIMConnectionStateMap = {
  'Android': <a href='@-ZIMConnectionStateMap' target='_blank'>ZIMConnectionStateMap</a>,
...
const config = {
    subscriptionDuration: 60, // in minutes
};
...
try {
    ...
    // Check for users with subscription failure
    if (result.errorUserList.length > 0) {
        result.errorUserList.forEach((errorUserInfo) => {
            console.log('User ID with subscription failure: ', errorUserInfo.userID);
...
zim.on('userStatusUpdated', function (zim, { userStatusList }) {
    userStatusList.forEach((userStatus) => {
        ...
        // Perform the logic after user status update here, such as updating UI or other operations
    });
});
```
:::
...
try {
    ...
    // Check for users who failed to unsubscribe
    if (result.errorUserList.length > 0) {
        result.errorUserList.forEach((errorUserInfo) => {
            console.log('User ID failed to unsubscribe: ', errorUserInfo.userID);
            console.log('Error code for failed unsubscribe: ', errorUserInfo.reason);
        });
    } else {
        console.log('All users unsubscribed successfully!');
    }
} catch (error) {
...
try {
    ...
    result.userStatusList.forEach((userStatus) => {
        ...
        console.log('Last Update Time:', userStatus.lastUpdateTime); // Last time the online status was updated
    });

    // Check for failed user queries
    result.errorUserList.forEach((errorUserInfo) => {
        console.log('Failed User ID:', errorUserInfo.userID);
        console.log('Error Code:', errorUserInfo.reason);
    });
} catch (error) {
    // Catch and handle errors
