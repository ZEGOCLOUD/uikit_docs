...
Before getting the conversation list, make sure that the following conditions are met:
...
You can get a user's conversation list by calling the server API. For more details, please refer to the server API documentation [Query conversation list](!zim-QueryConversationList).
