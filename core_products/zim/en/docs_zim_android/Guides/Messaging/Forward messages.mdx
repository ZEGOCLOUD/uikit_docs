...
Before implementing the "Forward messages" function, please make sure:
...
2. Call the {getPlatformData(props,sendMessageMap)} interface to send the merged message. For more information about this operation, see [Send & Receive messages - Send messages](!zim-sendMessages#4_1).
...
The callback interface for receiving merged messages is the same as the callback interface for receiving normal messages. Please refer to [Send & Receive messages - Receive messages](!zim-sendMessages#4_2) for the specific interface.
...
Forwarding messages one by one is essentially sending messages that have been successfully sent as parameters to other conversations. It uses the same interface as sending normal messages. Developers can refer to [Send & Receive messages - Send messages](!zim-sendMessages#4_1) for details of this interface.
