...
export const onRoomMemberAttributesUpdatedMap = {
  ...
  'iOS': <a href='/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-room-member-attributes-updated-infos-operated-info-room-id' target='_blank'>onRoomMemberAttributesUpdated</a>,
  'mac': <a href='/article/api?doc=zim_API~objective-c_macos~protocol~ZIMEventHandler#zim-room-member-attributes-updated-infos-operated-info-room-id' target='_blank'>onRoomMemberAttributesUpdated</a>,
}
...
| ---- | ----| -------| -----|
| attributes | {getPlatformData(props,HashMap)} | Yes | The configured user attribute. |
| roomID | {getPlatformData(props,stringMap)} | Yes | The ID of the room in which user attributes need to be configured. |
| userIDs | {getPlatformData(props,listMap)} | Yes | The IDs of the users whose attributes need to be configured. |
| config | {getPlatformData(props,ZIMRoomMemberAttributesSetConfigMap)} | Yes | <p>The configuration of a user attribute. `isDeleteAfterOwnerLeft` indicates whether the user attribute involved in this operation is automatically deleted after the owner leaves the room.</p><p><b> **If `config` is not configured, the user attribute is automatically deleted after the owner leaves the room by default.**</b></p> |
| callback |  {getPlatformData(props,ZIMRoomMemberAttributesListQueriedCallbackMap)} | Yes | The result callback of the user attribute operation. |
...
| Parameter | Type | Required | Description |
| ---- | ----| -------| -----|
| userIDs | {getPlatformData(props,listMap)} | Yes | The IDs of the users whose user attributes need to be queried. |
| roomID | {getPlatformData(props,stringMap)} | Yes | The ID of the room in which the user attributes need to be queried. |
| callback | {getPlatformData(props,ZIMRoomAttributesQueriedCallbackMap)} | Yes | The result callback for querying user attributes. |
#### Sample code
:::if{props.platform=undefined}
```java
ArrayList<String> userIDs = new ArrayList<>();
...
| Parameter | Type | Required | Description |
| ---- | ----| -------| -----|
| roomID | {getPlatformData(props,stringMap)} | Yes | The ID of the room in which the user attributes need to be configured. |
| config | {getPlatformData(props,ZIMRoomMemberAttributesQueryConfigMap)} | Yes | <p>The configuration of a user attribute. </p><ul><li>`nextFlag`: During the first query, a null character string is passed. During second and subsequent queries, `nextFlag` returned in the callback is passed. If `nextflag` returned in the callback is null, the query is completed.</li> <li>`count`: The number of users on one page during the page-based query.</li> </ul> |
| callback  | {getPlatformData(props,ZIMRoomMemberAttributesListQueriedCallbackMap)} | Yes | The result callback for querying user attributes of all users. After a successful query, user attributes of all users in the room are returned. |
