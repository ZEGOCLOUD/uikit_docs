...
![](https://storage.zego.im/sdk-doc/Pics/InappChat/im_solution_en.png)
...
Go to the `app` directory, open the `build.gradle` file, and add the following line to the `dependencies`. (**x.y.z** is the SDK version number, to obtain the latest version number, see [Release Note](#14167))
...
![/Pics/Android/ZIM/android_studio_im_sdk.jpg](https://storage.zego.im/sdk-doc/Pics/Android/ZIM/android_studio_im_sdk.jpg)
...
1. Call the {getPlatformData(props,ZIMUserInfoMap)}  method to create a user object.
2. Then, call the {getPlatformData(props,loginMap)} method with their own user information.
...
- For SDK 2.3.0 or later: The SDK uses the AppSign authentication mode by default. You only need to pass the {getPlatformData(props,ZIMUserInfoMap)} when logging in. 
...
After client B logs in, he will receive client A's message through the callback {getPlatformData(props,onReceivePeerMessageMap)} which is already set in the {getPlatformData(props,setEventHandlerMap)} method.
