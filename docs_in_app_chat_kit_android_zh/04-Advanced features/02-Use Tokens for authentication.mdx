...
如果你需要使用 Token 而不是 Appsign，您可以调用`initWith`而不带上`appsign`来完成初始化。
...
在大多数情况下，上述提到的令牌获取方法应该可以正常工作。然而，在某些极端情况下，它可能不可靠。

为了确保令牌的可靠性，您可以进行以下修改：

1. 令牌的有效期应尽可能设置得长，例如3天。
2. 监听`onTokenWillExpire`回调，并调用`renewToken`来更新新的令牌。
