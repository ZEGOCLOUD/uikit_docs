...
1. If you need to display a user's avatar, set the avatar image URL (PNG, JGP, JGEP, BMP, GIF, and WEBP are supported) to `{props.avatarKey}` in `ZegoUIKitPrebuiltLiveAudioRoomConfig`. This way, Live Audio Room Kit will render the avatar for you based on the URL, rather than using the default letters.
...
- Even when using `avatarBuilder`, you can still set the avatar URL to `{props.avatarKey}`. After setting this, Live Audio Room Kit will synchronize each user's `{props.avatarKey}` in the room for you, and provide this parameter in `avatarBuilder`. This way, you can directly render each user's avatar in `avatarBuilder`.
</Note>