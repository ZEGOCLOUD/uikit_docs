...
- 权限认证 Token：为了进一步提高安全性，开放房间 ID 和推流 ID 这两个权限位，可以验证登录房间的 ID 和推流 ID，此时 “payload” 字段需要按照规格生成。权限位校验规则如下：
...
- ZEGO Express SDK 6.9.0 及以上版本请使用 zego_server_assistant 中的 “token04” 来生成 Token。
</div>
...
<table>
  ...
  <tr>
    ...
    <td colspan="2"><ul><li><a target="_blank" href="https://github.com/zegoim/zego_server_assistant/blob/feature/token04/token/.net/token04/demo/WindowsFormsApp1/Form1.cs">GitHub</a></li><li><a target="_blank" href="https://gitee.com/zegodev_admin/zego_server_assistant/blob/feature/token04/token/.net/token04/demo/WindowsFormsApp1/Form1.c">Gitee</a></li></ul></td>
    <td></td>
  </tr>
  <tr>
    <td>Node.js</td>
