...
- Follow the integration steps by referring to the [Quick start with co-hosting](/live-streaming-kit-ios/quick-start/quick-start-(with-cohosting)).
...
To receive a PK battle request, you can [listen to](#receives-a-pk-battle-request) and set up the `onIncomingPKRequestReceived`.
