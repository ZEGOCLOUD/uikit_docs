...
To display gift animations to the host and viewers, send a notification using the [send in-room messages](https://www.zegocloud.com/docs/uikit/in-app-chat-ios/server-apis/send-in-room-messages) API with `MessageType: 2`. 
