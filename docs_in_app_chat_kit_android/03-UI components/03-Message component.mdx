...
In-app Chat Kit allows you to integrate the message page into your Activity as an Activity or a Fragment.
...
- **Method 1**: Directly redirect to Activity
...
- **Method 2**: Integrate the message page into your Activity as a Fragment

To integrate the message component into your Activity, state it in the `XML` file or load it by using `FragmentManager` in the Java code based on actual requirements. 

The following shows the sample code for the two methods: 

****Using FragmentManager in code****:
...
<Accordion title="Custom title bar" defaultOpen="false">

To customize buttons on the `TitleBar`, you can implement the `registerMessageListListener`.

For example, when you want to add a "Start a call" button on the upper right of the message list (`ZIMKitMessageActivity`). To learn more details, see <a href="../04-Advanced%20features/01-Use%20in%20conjunction%20with%20Call%20Kit.mdx" target="_blank">Use with the Call Kit</a>.
